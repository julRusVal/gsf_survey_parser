...
#based on GSF Version 3.05 2014
...
# from geodetic_survey import geodetic
...
def testreader():
	...
	while r.moreData():
		...
		if recordidentifier == SWATH_BATHYMETRY:
			...
			ptr.write( "%s %.9f %.9f %.9f %.9f %.9f \n" % (datagram.from_timestamp(datagram.timestamp), timeutils.unix2adjustedgps(datagram.timestamp), datagram.timestamp, datagram.longitude, datagram.latitude, datagram.heading))
...
class SWATH_BATHYMETRY_PING :
	...
	def read(self, previousscalefactors={}, headeronly=False):
		...
		while (self.fileptr.tell() < self.offset + self.numbytes): #dont read past the end of the packet length.  This should never happen!
			...
			# trap out corrupt end of file record.  we have seen this in the past
			if len(data) != subrecfmtlen:
				continue
			...
			if subrecord_id > len(self.scalefactorsd):
				sf = SCALEFACTOR()
				sf.subrecordID = subrecord_id
				sf.compressionFlag = 0
				sf.offset = 0
				sf.multiplier = 1
				# self.fileptr.seek(subrecord_size, 1) #move forwards to the end of teh record
				# continue
			...
			if subrecord_id == 1: 
				...
				# print ("QF ARRAY")
				self.QUALITY_FLAGS_ARRAY = self.readcompressedarray(sf.multiplier, sf.offset, datatype, subrecord_id, subrecord_size)
		...
		# if the beam flags is empty we need to fake it
		if len(self.BEAM_FLAGS_ARRAY) == 0:
			self.BEAM_FLAGS_ARRAY = self.zerolistmaker(len(self.DEPTH_ARRAY))
	...
	###############################################################################
	def readcompressedarray(self, scale, offset, datatype, subrecord_id, subrecord_size):
		''' 
		read the ping array data of COMPRESSED records.  we then unpack these into the real array layout
		'''
		fmt = '>' + datatype
		nparr = np.fromfile(self.fileptr, dtype=np.dtype(fmt), count=subrecord_size)
		nparr = (nparr / scale) - offset
		# make nparr lengtht the same as the number of beams
		nparr = np.repeat(nparr, self.numbeams / subrecord_size)
		# nparr = (nparr / scale) + offset  we should be subtracting not adding!!!!
		return nparr
...
###############################################################################
	def getapproximatepositon(self):
		...
		self.fileptr.seek(curr, 0)
		self.rewind()
		return datagram.longitude, datagram.latitude
